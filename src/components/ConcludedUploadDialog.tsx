      ...
      try {
        if (row.data.type === 'bill') {
          await addBill({
            ...
            initialStatus: row.data.status, // Pass 'tbd', 'pending', or 'concluded'
            concludedAt: row.data.concludedAt
          });
        } else {
          await addDocument({
            title: row.data.title,
                            ...
                            {!result.valid ? (
                              ...
                              <>
                                 <div className="flex items-center gap-2">
                                     ...
                                     {result.data.status === 'tbd' && (
                                         <span className="px-1.5 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider bg-slate-200 text-slate-700">
                                             TBD
                                         </span>
                                     )}
                                     {result.data.status === 'concluded' && (
                                         <span className="px-1.5 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider bg-green-200 text-green-700">
                                             CONCLUDED
                                         </span>
                                     )}
                                 </div>
